...
import com.journey.passenger_presenter.databinding.ItemCutListBinding
import com.journey.passenger_presenter.databinding.ItemDeviceBinding
    ...
    inner class ViewHolderDevice(private var binding: ItemDeviceBinding) : ViewHolder(binding) {
        ...
        override fun bind(item: DataBluetoothList) {
            if (item is DataDevice) {
                ...
                if (item.isConnected) binding.tvName.setTextColor(ContextCompat.getColor(context, com.journey.common_utils.R.color.blue))
            }
        }
    }
}